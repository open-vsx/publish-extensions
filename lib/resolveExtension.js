 ...
 */
exports.resolveExtension = async function ({ id, repository, location, helper }, ms) {
  ...
  if (helper) {
    const resolveJupyter = require("../helpers/"+helper).resolveVersion;
    const extraLookup = await resolveJupyter(ms.version, ms?.lastUpdated);
    if (extraLookup) {
      return extraLookup;
    }
  }
  ...
  if (ms && repositoryUrl.hostname === 'github.com' && token) {
    try {
      ...
      if (Object.keys(platformSpecific).length > 0) {
        return { version: ms.version, files: platformSpecific, path: '', resolution: { releaseAsset: 'resolved' } };
      }
