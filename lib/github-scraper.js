...
exports.findLatestVSIXRelease = async function (repository) {
  ...
  if (releaseData.data.repository.releases.edges.length === 0) {
    throw new Error(`${owner}/${repo} does not have any releases`);
  }
  ...
  .filter((release) => !release.node.isPrerelease)
    ...
    throw new Error("No suitable releases found");
  }
  ...
  if (suitableReleases.length === 0) {
    throw new Error("No suitable release assets found");
  }
  return await suitableReleaseAssets[0].downloadUrl;
}
